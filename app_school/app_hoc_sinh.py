...
def bang_diem_hs():
    if session.get("hocsinh") is None:
        return redirect(url_for('index'))
    ...
    for i in bangdiem:
        i.IDMon = Ten_Mon(i.IDMon)  # Assuming Ten_Mon is a function that returns subject name based on ID
        if i.TrungBinhMon is not None:
            DiemTB += i.TrungBinhMon
            so_mon += 1
    
   
    DiemTB = DiemTB / so_mon if so_mon > 0 else 0
